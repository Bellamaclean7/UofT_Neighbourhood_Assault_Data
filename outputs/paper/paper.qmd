---
title: "How Safe is Campas Really? Assault Rate Trends Within and Around the University of Toronto: A Ten-Year Analysis"
...
## Data
The data utilized throughout this paper was obtained through the City of Toronto’s OpenDataToronto Library [@rOpenDataToronto]. The data set used is entitled 'Neighbourhood Crime Rates' [@citeTorontoCrimeRates]. Data was collected and analyzed using the statistical programming software R [@r], with additional support from `tidyverse` [@rTidyverse], `ggplot2` [@rGgplot2], `dplyr` [@rDplyr], `readr` [@rReadr], `KableExtra` [@rKableExtra], `tibble` [@rTibble], `janitor` [@rJanitor], and `knitr` [@rknitr].
...
ggplot(long_data, aes(x = Year, y = Assault_Rate, group = Neighbourhood, color = Neighbourhood)) +
  ...
  ylim(0, 3000) # Adjust these values based on my data's range
...
asst_data <- surrounding_neighbourhoods |>
  gather(key = "Year", value = "Assault_Rate", -Neighbourhood) |>
  mutate(Year = as.numeric(str_extract(Year, "\\d{4}")))

# Checking for NA values in Year because when I was building my graph orignally the output was a graph with nothing plotted on it even though everything else looked good the Year column is producing NA values
...
ggplot(long_data, aes(x = Year, y = Assault_Rate, group = Neighbourhood, color = Neighbourhood)) +
  ...
  theme_minimal() +
  labs(
    title = "Assault Rates in University of Toronto's Surrounding Neighbourhoods (2014-2023)",
    x = "Year",
    y = "Assault Rate"
  ) +
